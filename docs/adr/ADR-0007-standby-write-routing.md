...
## Статус
🛑 **Superseded (2026-09-12) — замінено [ADR-0008: Рівноправні read-write вузли + центральний хаб синхронізації](ADR-0008-peer-nodes-sync-hub.md).**

ADR-0008 відмовляється від фізичної реплікації: кожен вузол стає повним
read-write PostgreSQL, обмін даними — прикладним HTTP-протоколом
(`POST /api/v1/sync/push`, `GET /api/v1/sync/master`), центральний хаб — арбітр
спільних довідників. Рубежі 1/2 цього ADR (перехоплення запису на
standby/read-only репліці) стають непотрібними і видаляються окремим
ізольованим етапом **E7** плану
[`docs/architecture/plan-adr0008-peer-nodes.md`](../architecture/plan-adr0008-peer-nodes.md).

Цей файл зберігається як **історичний запис** (рішення, факти з логів
продакшн-каси, прийняті компроміси) — НЕ видаляти.

### Історичний статус
✅ Прийнято (2026-09-11) — рішення Творця, заморожений інтерфейс.
→ 🛑 Superseded (2026-09-12) — див. ADR-0008 вище.
