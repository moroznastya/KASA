# Детальний план реалізації ПРРО для Torgashka
...
**Мета:** реалізувати в Torgashka програмний реєстратор розрахункових операцій (ПРРО)
...
2. У Torgashka: вікно роботи з ПРРО (операції) + налаштування ПРРО (електронний ключ + пароль).
...
## 2. Поточний стан Torgashka (що вже є / чого немає)
...
## 3. Архітектура ПРРО-модуля (Clean Architecture Torgashka)
...
### ДСТУ 4145-2002 підпис реалізовано! (2026-08-01, ~18:50)

**Коміти:**
- `23d3ee6` feat(prro): implement DSTU 4145-2002 signing via IIT SDK (EUSignCP)
- `f58b3ed` docs: план оновлено — test-connection повертає -13

**Результат:** `POST /api/v2/prro/test-connection` з JKS-ключем
(`pb_3791505547 (2).jks`, ПриватБанк, ДСТУ 4145) тепер повертає:

```
status: -13  →  ERROR_NOT_REGISTERED_RRO: «ПРРО не зареєстровано»
```

**Це прогрес:** сервер ДПС РОЗПІЗНАВ підписаний XML (ДСТУ 4145 + ГОСТ 34311,
ІІТ SDK EUSignCP) — помилки -1 ERROR_VEREFY більше немає.

**Що зроблено:**
- ІІТ SDK розпаковано в `backend/vendor/iit-sdk/` (opt/iit/eu/sw, euscp.so)
- `crypto_signer` підписує через EUSignCP (ctypes): ДСТУ 4145-2002 + Стрибог-256
- JKS-ключ читається (обхід pyjks), OID ДСТУ визначається
- 454 тести зелених

**Наступний крок (потрібна дія користувача):**
1. Зареєструвати ПРРО в **тестовому кабінеті платника** ДПС
   (`https://cabinet.tax.gov.ua`) — отримати **ФН (фіскальний номер) ПРРО**.
2. Ввести ФН / заводський № / податковий № в Налаштуваннях ПРРО програми
   (поля prro_fn / prro_tn / prro_zn).
3. Повторно «Перевірити з'єднання» → очікуємо `status: 1` (успіх).
4. Відкриття зміни → фіскалізація → Z-звіт на тестовому API.

**⚠️ 500-помилка з програми усунена:** тепер помилки повертаються
зрозумілим JSON (HTTP 200), а не падінням з 500 Internal Server Error.
