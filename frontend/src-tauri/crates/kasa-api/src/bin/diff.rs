// ─────────────────────────────────────────────────────────────────────────────
// kasa-api — bin/diff.rs: differential CLI
// ─────────────────────────────────────────────────────────────────────────────
// Приймає JSON зі stdin: {"op":"...","args":{...}}
// Викликає відповідну функцію крейту kasa-api, повертає normalized JSON:
//   {"op":"...","ok":true|false,"result":...,"error":null|"..."}
//
// Операції (етап 0):
//   health  → kasa_api::health_payload()   → {"status":"ok"}
//   echo    → kasa_api::echo_payload(args) → args без змін
//
// Приклад:
//   echo '{"op":"echo","args":{"a":1}}' | cargo run -p kasa-api --bin diff
// ─────────────────────────────────────────────────────────────────────────────
...
fn dispatch(req: &DiffRequest) -> DiffResponse {
    match req.op.as_str() {
        "health" => DiffResponse {
            op: req.op.clone(),
            ok: true,
            result: kasa_api::health_payload(),
            error: None,
        },
        "echo" => DiffResponse {
            op: req.op.clone(),
            ok: true,
            result: kasa_api::echo_payload(&req.args),
            error: None,
        },
        other => DiffResponse {
            op: req.op.clone(),
