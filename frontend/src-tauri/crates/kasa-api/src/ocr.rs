...
pub async fn analyze_invoice(
    ...
    let ocr = state.ocr.as_ref().ok_or_else(|| {
        (
            ...
            "OCR не увімкнено (KASA_RUST_OCR=1)".to_string(),
        )
    })?;
    ...
    match ocr.analyze_invoice_image(&image_data).await {
        ...
        Err(kasa_ocr::OcrError::Runtime(msg)) => {
...
pub async fn analyze_with_matching(
    ...
    let repo = kasa_infrastructure::ocr::SqlxOcrRepository::new(pool);
    let service = kasa_ocr::InvoiceOcrService::new(repo);
