...
use kasa_application::DocumentsServiceFacade;
use kasa_domain::{BatchConfirmInput, DocListQuery, DocumentsError, DocumentsService, ExportQuery};
...
/// від KASA_RUST_AUTH: перевіряє роль через documents-пул (той самий PostgreSQL).
async fn require_admin_docs(state: &AppState, claims: &Claims) -> Result<Uuid, DocErr> {
    ...
    let user_id = Uuid::parse_str(&claims.sub).map_err(|_| {
        DocErr::Auth(AuthRouteError::Plain(kasa_domain::AuthError::Unauthorized(
...
) -> Result<Json<kasa_domain::DocListDto>, DocErr> {
...
) -> Result<Json<kasa_domain::BatchConfirmResultDto>, DocErr> {
...
pub async fn print(
...
) -> Result<Json<kasa_domain::DocPrintDto>, DocErr> {
    ...
    if claims.sub.is_empty() {
        return Err(DocErr::Auth(AuthRouteError::Plain(
            kasa_domain::AuthError::Unauthorized(
                "Недійсний токен: відсутній ідентифікатор користувача".to_string(),
            ),
        )));
    }
