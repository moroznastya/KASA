...
use kasa_domain::{CategoryCreateInput, CategoryDto, CategoryUpdateInput, WriteError};
...
impl From<kasa_domain::DirectoryError> for CatV2Error {
    fn from(e: kasa_domain::DirectoryError) -> Self {
        match e {
            kasa_domain::DirectoryError::NotFound(msg) => {
...
fn read_repo(
    state: &AppState,
) -> Result<std::sync::Arc<dyn kasa_domain::ReadDirectories + Send + Sync>, CatV2Error> {
    state
        .readdirs
        .clone()
...
fn write_repo(
    state: &AppState,
) -> Result<std::sync::Arc<dyn kasa_domain::WriteDirectories + Send + Sync>, CatV2Error> {
    state
        .write
        .clone()
        .ok_or_else(|| CatV2Error::Forbidden("Rust-гілка довідників вимкнена".to_string()))
