...
use kasa_domain::{DirectoryError, SupplierProductMovementsResponse, SupplierProductsResponse};
...
fn read_repo(
    state: &AppState,
) -> Result<std::sync::Arc<dyn kasa_domain::ReadDirectories + Send + Sync>, SupplierError> {
    state
        .readdirs
        .clone()
