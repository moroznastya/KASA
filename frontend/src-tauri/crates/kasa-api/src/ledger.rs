...
use kasa_application::services::ledger::LedgerServiceFacade;
use kasa_domain::{LedgerEntriesQuery, LedgerEntryInput, LedgerError, LedgerService as LedgerPort};
...
) -> Result<(StatusCode, Json<kasa_domain::LedgerEntryV1Dto>), LedgerErr> {
...
pub async fn balance_v1(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<kasa_domain::LedgerBalanceV1Dto>, LedgerErr> {
...
) -> Result<Json<kasa_domain::LedgerHistoryV1Dto>, LedgerErr> {
    let id = path_uuid(id, "supplier_id")?;
...
) -> Result<Json<kasa_domain::LedgerListV2Dto>, LedgerErr> {
...
pub async fn create_entry_v2(
    State(state): State<AppState>,
    Json(body): Json<Value>,
) -> Result<(StatusCode, Json<kasa_domain::LedgerEntryV2Dto>), LedgerErr> {
...
pub async fn balance_v2(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<kasa_domain::LedgerBalanceV2Dto>, LedgerErr> {
    let id = path_uuid(id, "supplier_id")?;
    let repo = ledger_repo(&state)?;
    let svc = LedgerServiceFacade::new(repo);
...
pub async fn all_balances_v2(
    State(state): State<AppState>,
) -> Result<Json<Vec<kasa_domain::SupplierBalanceV2Dto>>, LedgerErr> {
    let repo = ledger_repo(&state)?;
    let svc = LedgerServiceFacade::new(repo);
