...
use kasa_domain::{ProductsV2Error, ProductsV2Service};
...
pub async fn add_barcode(
    State(state): State<AppState>,
    AxumPath(product_id): AxumPath<String>,
    Json(body): Json<kasa_domain::BarcodeCreateV2Input>,
) -> Result<Json<Value>, ProductsV2ApiError> {
    let product_id = path_uuid(&product_id, "product_id")?;
...
pub async fn create_product(
    ...
    let input = kasa_domain::ProductCreateV2Input {
        name,
        barcode: body
            .get("barcode")
            .and_then(|v| v.as_str())
...
pub async fn update_product(
    ...
    let input = kasa_domain::ProductUpdateV2Input {
