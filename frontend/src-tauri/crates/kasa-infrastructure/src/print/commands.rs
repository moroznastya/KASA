...
pub fn print_image(
    ...
    eprintln!(
        "[KASA] print_image: base64_len={}, printer={:?}, device={:?}, copies={}, auto_cut={}, width_mm={:?}, height_mm={:?}, dpi={:?}",
...
pub fn print_raster_image(
    ...
    eprintln!(
        "[KASA] print_raster_image: data_len={}, printer={:?}, device={:?}, copies={}, auto_cut={}",
        image_data.len(),
        printer_name,
        device_path,
        copies,
...
//   кастомний URI-протокол `kasa-print://` (реєструється ОДИН раз у lib.rs
...
//     2. Створюється WebviewWindow з URL `kasa-print://localhost/{token}/` —
...
/// Реєстр HTML-документів для кастомного протоколу `kasa-print://`.
///
/// Ключ — унікальний токен (uuid), значення — HTML-документ для друку.
/// Використання токена в URL (`kasa-print://localhost/{token}/`) унеможливлює
...
///      `kasa-print://localhost/{token}/` — кастомний протокол, який
...
pub fn print_html(app: tauri::AppHandle, data: PrintHtmlData) -> Result<PrintResult, String> {
    ...
    eprintln!(
        "[KASA] print_html: html_len={}, printer={:?}, window='{}'",
        data.html.len(),
        data.printer_name,
        label
    );

    // ── 1. Зберігаємо HTML у реєстрі для протоколу kasa-print:// ────────
    store_print_html(&token, data.html);

    // ── 2. Формуємо URL кастомного протоколу ─────────────────────────────
    // webkit2gtk зробить запит на kasa-print://localhost/{token}/ —
    // хендлер (зареєстрований у lib.rs) поверне HTML з реєстру за токеном.
    let url = tauri::Url::parse(&format!("kasa-print://localhost/{}/index.html", token))
    ...
    eprintln!(
        "[KASA] ✅ print_html: вікно '{}' створено, діалог друку викликається",
        label
    );
