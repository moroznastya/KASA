...
use kasa_domain::{
    iso_naive, iso_utc_z, parse_scaled2, parse_scaled3, DocItemInput, MySessionsDto, PosError,
...
/// Повертає "0.0" (Python float-джерело default) або "0.00" (scale БД).
/// create → "0.0"; read → "0.00".
fn zero_amount(create: bool) -> String {
    if create {
        "0.0".to_string()
    } else {
        "0.00".to_string()
...
async fn fetch_receipt_dto(
    ...
    Ok(ReceiptDto {
        ...
        fiscal_sent_at: row.get("fiscal_sent_at"),
...
async fn fetch_receipt_items_short(
    ...
    let rows = sqlx::query(
        "SELECT product_id, quantity::text, price::text FROM receipt_items WHERE receipt_id = $1 ORDER BY created_at",
    )
    .bind(receipt_id)
    .fetch_all(pool)
    .await
...
async fn read_write_off_dto(
    ...
    let items = sqlx::query(
        "SELECT id, write_off_id, product_id, quantity::text, cost_price::text, price::text, \
         created_at::text FROM write_off_items WHERE write_off_id = $1 ORDER BY created_at",
    )
    .bind(id)
    .fetch_all(pool)
    .await
...
async fn build_write_off_create(
    ...
    for (item, cost, price) in items {
        let row = sqlx::query(
            "SELECT id, created_at::text FROM write_off_items WHERE write_off_id = $1 AND product_id = $2 ORDER BY created_at DESC LIMIT 1",
        )
        .bind(id)
        .bind(item.product_id)
        .fetch_one(pool)
    ...
    Ok(WriteOffDto {
        ...
        number,
        reason,
        write_off_date: iso_naive(date),
        notes,
        status: "confirmed".to_string(),
        total_amount: Some(zero_amount(true)),
        created_at: iso_naive(created),
        updated_at: iso_naive(updated),
        items: dtos,
    })
...
impl PosService for SqlxPos {
    ...
    async fn create_write_off(&self, input: &WriteOffCreateInput) -> Result<WriteOffDto, PosError> {
        ...
        let row = sqlx::query(
            ...
            VALUES ($1, $2, $3::write_off_reason, $4, $5, $6, (now() AT TIME ZONE 'UTC')::timestamp,
                (now() AT TIME ZONE 'UTC')::timestamp)
        ...
        // Авто-confirm: зменшуємо залишки (DocumentService.confirm_write_off).
        for item in &input.items {
            let qty = parse_scaled3(&item.quantity).unwrap_or(0);
    ...
    async fn list_shifts(&self, page: i64, size: i64) -> Result<ShiftListDto, PosError> {
        ...
        for r in rows {
            ...
            items.push(PrroShiftDto {
                ...
                opened_at: iso_naive(opened),
                closed_at: closed.map(iso_naive),
