...
fn products_to_dicts(products: &[kasa_domain::print::PrintProductInput]) -> Vec<Value> {
    products
        .iter()
        .map(|p| {
            json!({
...
impl kasa_domain::print::PrintTemplatesService for SqlxPrintTemplates {
...
mod tests {
    ...
    fn render_replace_basic() {
        let data = json!({"shop_name": "Kasa", "total": "100.00"});
        assert_eq!(
            render_replace("<h1>{{shop_name}}</h1><p>{{total}} грн</p>", &data),
            "<h1>Kasa</h1><p>100.00 грн</p>"
        );
    }
