...
//!   реальні дані користувача (~/.local/share/torgashka/offline.db) не чіпаються.
...
fn print_receipt_to_mock_device() {
    ...
    let mock_dev = std::env::temp_dir().join("torgashka_mock_print.bin");
    ...
    torgashka_infrastructure::print::print_raster_image(
        png,
...
fn offline_queue_roundtrip_on_disk() {
    ...
    // <tmp>/torgashka/offline.db (реальний файл SQLite, WAL).
