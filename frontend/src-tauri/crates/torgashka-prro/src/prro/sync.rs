...
//! (спека prro_fix_spec_2026-09-08, D/G).
...
impl SyncOfflineQueueUseCase {
    ...
    ///
    /// Спека ПРРО:
    ///  - кожен документ передається зі своїм id_offline (резервний
    ///    фіскальний номер офлайн-чека) — він НЕ губиться при повторній
    ///    передачі (спека G);
    ///  - документи без check_sign формуються рівно 1 раз: повний RQ з тим
    ///    самим MAC/ID, що й при створенні (build_message ≤ 1 разу на
    ///    документ, MAC не змінюються);
    ///  - після успішної передачі shift.last_mac = hash(повного RQ
    ///    документа) — наступний Check посилається на цей хеш (спека D).
    ...
    pub async fn sync(
        ...
        for item in &pending {
            ...
            let id_offline = item.id_offline.clone().unwrap_or_default();
            let outcome = async {
                let signed: Vec<u8> = match &item.check_sign {
                    Some(cs) if !cs.is_empty() => xml::signed_text_to_bytes(cs)
                        .map_err(|e| PrroShiftError::new(e.to_string(), "XML_BUILD_ERROR"))?,
                    _ => {
                        // Повний RQ з тим самим MAC/ID, що й при створенні документа
                        // (спека G: id_offline → ID тега <MAC> офлайн-чека).
                        let mac_value = item.mac.clone().unwrap_or_default();
                        let message = xml_builder
                            .build_message(&item.xml_body, Some(&mac_value), &id_offline, true)
                            .map_err(|e| PrroShiftError::new(e.to_string(), "XML_BUILD_ERROR"))?;
                        let signed = signer
                            .sign(&xml::cp1251_bytes(&message).map_err(|e| {
                                PrroShiftError::new(e.to_string(), "XML_BUILD_ERROR")
                            })?)
                            .map_err(|e| PrroShiftError::new(e.to_string(), "SIGN_ERROR"))?;
                        PrroOfflineQueue::update_check_sign(
                            repo,
                            item.id,
                            xml::signed_bytes_to_text(&signed),
                        )
                        .await
                        .map_err(|e| PrroShiftError::new(e.to_string(), "QUEUE_ERROR"))?;
                        signed
                ...
                let check = Check {
                    ...
                    id_offline: id_offline.clone(), // G: id_offline не губиться при sync
            ...
            match outcome {
                Ok(response) if response.status == 1 => {
                    ...
                    // наступний Check посилатиметься на цей хеш (ланцюг не рветься).
                    if let Some(shift_id) = item.shift_id {
                        let mac_value = item.mac.clone().unwrap_or_default();
                        let chain_message =
                            xml::reconstruct_full_rq(&item.xml_body, &mac_value, &id_offline)
                                .map_err(|e| {
                                    PrroShiftError::new(e.to_string(), "XML_BUILD_ERROR")
                                })?;
                        let next_mac = xml::compute_mac(&chain_message)
                            .map_err(|e| PrroShiftError::new(e.to_string(), "XML_BUILD_ERROR"))?;
                        repo.update_shift_last_mac(shift_id, next_mac)
                            .await
                            .map_err(|e| PrroShiftError::new(e.to_string(), "QUEUE_ERROR"))?;
                    }
                    result.synced += 1;
