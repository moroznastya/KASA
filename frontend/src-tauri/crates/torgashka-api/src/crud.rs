...
//
// Монтуються лише під feature-flag TORGASHKA_RUST_READDIRS=1; інакше всі ці
// шляхи йдуть у fallback → 410 (дезактивація).
// ─────────────────────────────────────────────────────────────────────────────
...
use torgashka_application::{ReadDirectoryService, ServiceError, WriteService};
    ...
    fn into_response(self) -> Response {
        match self {
            ...
            CrudError::Service(e) => match e {
                ServiceError::Write(torgashka_domain::WriteError::NotFound(msg)) => (
                    StatusCode::NOT_FOUND,
                    Json(serde_json::json!({"detail": msg})),
                )
                    .into_response(),
                ServiceError::Write(torgashka_domain::WriteError::Conflict(msg)) => (
                    StatusCode::CONFLICT,
                    Json(serde_json::json!({"detail": msg})),
                )
                    .into_response(),
                ServiceError::Write(torgashka_domain::WriteError::BadRequest(msg)) => (
                    StatusCode::BAD_REQUEST,
                    Json(serde_json::json!({"detail": msg})),
                )
                    .into_response(),
                ServiceError::Write(torgashka_domain::WriteError::Forbidden(msg)) => (
                    StatusCode::FORBIDDEN,
                    Json(serde_json::json!({"detail": msg})),
                )
                    .into_response(),
                ...
                ServiceError::Directory(torgashka_domain::DirectoryError::NotFound(msg)) => (
                    ...
                    .into_response(),
                ServiceError::Directory(torgashka_domain::DirectoryError::InvalidUuid {
                    field,
                    value,
                }) => (
                    StatusCode::BAD_REQUEST,
...
fn read_repo(
    state: &AppState,
) -> Result<std::sync::Arc<dyn torgashka_domain::ReadDirectories + Send + Sync>, CrudError> {
    state
        .readdirs
        .clone()
...
fn write_repo(
    state: &AppState,
) -> Result<std::sync::Arc<dyn torgashka_domain::WriteDirectories + Send + Sync>, CrudError> {
    state
        .write
        .clone()
        .ok_or_else(|| CrudError::Forbidden("Rust-гілка довідників вимкнена".to_string()))
...
pub async fn get_product(
    ...
    Path(id): Path<String>,
) -> Result<Json<torgashka_domain::ProductDto>, CrudError> {
...
) -> Result<(StatusCode, Json<torgashka_domain::ProductDto>), CrudError> {
...
pub async fn get_category(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<torgashka_domain::CategoryDto>, CrudError> {
...
pub async fn create_category(
    ...
    Extension(claims): Extension<Claims>,
    Json(body): Json<Value>,
) -> Result<(StatusCode, Json<torgashka_domain::CategoryDto>), CrudError> {
    require_admin(&state, &claims).await?;
...
pub async fn get_supplier(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<torgashka_domain::SupplierDto>, CrudError> {
...
pub async fn list_all_suppliers(
    State(state): State<AppState>,
) -> Result<Json<Vec<torgashka_domain::SupplierDto>>, CrudError> {
    let repo = read_repo(&state)?;
    let svc = ReadDirectoryService::new(repo);
...
pub async fn create_supplier(
    ...
    Extension(claims): Extension<Claims>,
    Json(body): Json<Value>,
) -> Result<(StatusCode, Json<torgashka_domain::SupplierDto>), CrudError> {
    require_admin(&state, &claims).await?;
...
pub async fn inventory_counts(
    State(state): State<AppState>,
) -> Result<Json<torgashka_domain::InventoryCountsDto>, CrudError> {
    let repo = write_repo(&state)?;
    let svc = WriteService::new(repo);
...
pub async fn get_inventory(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<torgashka_domain::InventoryDto>, CrudError> {
...
pub async fn create_inventory(
    ...
    Extension(claims): Extension<Claims>,
    Json(body): Json<Value>,
) -> Result<(StatusCode, Json<torgashka_domain::InventoryDto>), CrudError> {
    let user_id = Uuid::parse_str(&claims.sub).map_err(|_| {
        CrudError::Unauthorized("Недійсний токен: відсутній ідентифікатор користувача".to_string())
    })?;
    require_admin(&state, &claims).await?;
...
pub async fn confirm_inventory(
    ...
    match status {
        ...
        "draft" => Err(CrudError::Service(ServiceError::Write(
            torgashka_domain::WriteError::BadRequest(
