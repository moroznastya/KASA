...
use torgashka_application::services::auth::AuthServiceFacade;
...
pub(crate) fn auth_repo(
    ...
    state.auth.clone().ok_or_else(|| {
        AuthError::Forbidden("Rust-гілка auth вимкнена (TORGASHKA_RUST_AUTH=0)".to_string()).into()
    })
}
...
) -> Result<Json<torgashka_domain::LoginResult>, AuthRouteError> {
...
pub async fn users_list(
    State(state): State<AppState>,
) -> Result<Json<Vec<torgashka_domain::PublicUserDto>>, AuthRouteError> {
    let repo = auth_repo(&state)?;
    let svc = AuthServiceFacade::new(repo);
...
) -> Result<Json<torgashka_domain::UserListDto>, AuthRouteError> {
...
pub async fn settings_all(
    State(state): State<AppState>,
    Extension(claims): Extension<Claims>,
) -> Result<Json<torgashka_domain::SettingsModulesDto>, AuthRouteError> {
    let repo = auth_repo(&state)?;
    let svc = AuthServiceFacade::new(repo);
