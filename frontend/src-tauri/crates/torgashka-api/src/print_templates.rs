...
// Монтуються лише під TORGASHKA_RUST_PRINT=1; інакше — fallback → 410 (дезактивація).
// ─────────────────────────────────────────────────────────────────────────────
...
/// TORGASHKA_RUST_AUTH: перевіряє роль через print-пул.
...
fn svc(state: &AppState) -> Result<&dyn torgashka_domain::print::PrintTemplatesService, PrintErr> {
...
async fn list_printers_public() -> Vec<String> {
    torgashka_infrastructure::repositories::print_templates::list_printers().await
}
...
pub async fn render_template(
    ...
    Path(id): Path<Uuid>,
    Extension(_claims): Extension<Claims>,
    Json(body): Json<torgashka_domain::print::TemplateRenderInput>,
) -> Result<Json<Value>, PrintErr> {
    let s = svc(&state)?;
    Ok(Json(
