...
// Монтуються лише під TORGASHKA_RUST_DEBTORS=1; інакше — fallback → 410 (дезактивація).
// ─────────────────────────────────────────────────────────────────────────────
...
use torgashka_application::DebtorServiceFacade;
...
) -> Result<Json<Vec<torgashka_domain::DebtorDto>>, DebtErr> {
...
) -> Result<Json<torgashka_domain::DebtorListDto>, DebtErr> {
...
) -> Result<(StatusCode, Json<torgashka_domain::DebtorDto>), DebtErr> {
...
pub async fn get(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<torgashka_domain::DebtorDto>, DebtErr> {
...
pub async fn receipts(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<Vec<torgashka_domain::DebtorReceiptDto>>, DebtErr> {
    let id = path_uuid(id, "debtor_id")?;
    let repo = debtor_repo(&state)?;
    let svc = DebtorServiceFacade::new(repo);
...
pub async fn payments(
    State(state): State<AppState>,
    Path(id): Path<String>,
) -> Result<Json<Vec<torgashka_domain::DebtorPaymentDto>>, DebtErr> {
    let id = path_uuid(id, "debtor_id")?;
    let repo = debtor_repo(&state)?;
    let svc = DebtorServiceFacade::new(repo);
...
fn validate_decimal_amount(amount: &str) -> Result<(), DebtErr> {
    ...
    let cents = torgashka_domain::parse_scaled2(amount).unwrap_or(0);
    if neg || cents <= 0 {
        return Err(DebtErr::Validation(serde_json::json!({
            "detail": [{
                "type": "greater_than",
