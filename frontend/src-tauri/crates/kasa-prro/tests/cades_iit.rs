...
use kasa_prro::crypto::{default_iit_sdk_path, signer_from_key_material};
...
fn cades_signer_serial_and_name_match_python() {
    ...
    let mut sdk = kasa_prro::crypto::iit::IitSdk::load(&so).expect("SDK завантажується");
    sdk.initialize(None, &kasa_prro::crypto::default_iit_cert_store().unwrap())
        .expect("EUInitialize");
...
fn cades_sign_verify_and_structure() {
    ...
    let mut sdk = kasa_prro::crypto::iit::IitSdk::load(&so).expect("SDK");
    sdk.initialize(None, &kasa_prro::crypto::default_iit_cert_store().unwrap())
        .expect("init");
