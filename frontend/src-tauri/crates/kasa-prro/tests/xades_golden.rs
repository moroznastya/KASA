...
use kasa_prro::crypto::{PrroSigner, XadesKey, XadesSigner};
use kasa_prro::keystore;

fn load_signer() -> XadesSigner {
    let Some(pem_path) = kasa_prro::crypto::xades::test_rsa_key_path() else {
...
fn xades_signer_identity_matches_python() {
    ...
    //         name="Тестовий Підписант Kasa" (з certs/prro-test/test-rsa.pem)
    assert_eq!(
